...
class Server:
    def __init__(self):
        ...
        self.sender.bind(('localhost', 9090))
    ...
    def process_command(self, command: str, addr):
        ...
        if command.startswith('REGISTER'):
            ...
            if res:
                self.sender.sendto(bytes('ok', encoding='utf-8'), addr)
                print(f'Registered user: {user_login}, {user_password}, {user_name}')
            else:
                print('Rejected registration')
                self.sender.sendto(bytes('bad', encoding='utf-8'), addr)
        ...
        if command.startswith('LOGIN'):
            ...
            if res:
                ...
                self.sender.sendto(bytes('%'.join(str(i) for i in res), encoding='utf-8'), addr)
            else:
                print('Rejected login')
                self.sender.sendto(b'bad', addr)
        ...
        if command.startswith('PARTICIPATE'):
            ...
            print(f'Participated {user_login}, {warrior_name} to {game_id} ')
        ...
        if command.startswith('RECEIVE_DATA'):
            ...
            res = self.games[int(game_id)].participants[user_login].get_data()
        ...
        if command.startswith('START'):
            ...
            for user in self.games[int(game_id)].participants.values():
                self.sender.sendto(b'game started', user.user_addr)
